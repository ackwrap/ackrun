...
func RegisterRoutes(
	...
	v1.Use(configMutationMiddleware(reconcileSvc))
	{
		...
		v1.GET("/settings/general", settingsH.GetGeneralSettings)
		v1.PUT("/settings/general", settingsH.SetGeneralSettings)
...
func registerSSHHostRoutes(group *gin.RouterGroup, sshHostH *handler.SSHHostHandler) {
	...
	group.POST("/advanced/ssh/hosts/share", sshHostH.ShareHosts)
	...
	group.POST("/advanced/ssh/hosts/:id/sing-box/install", sshHostH.InstallSingbox)
	...
	group.GET("/advanced/ssh/sessions/:sessionID/details", sshHostH.GetSessionDetails)
	...
	group.POST("/advanced/ssh/sessions/:sessionID/sftp/copy", sshHostH.CopySFTP)
	group.GET("/advanced/ssh/sessions/:sessionID/sftp/text", sshHostH.ReadSFTPText)
	group.PUT("/advanced/ssh/sessions/:sessionID/sftp/text", sshHostH.WriteSFTPText)
	...
	group.GET("/advanced/ssh/sessions/:sessionID/sftp/download", sshHostH.DownloadSFTP)
