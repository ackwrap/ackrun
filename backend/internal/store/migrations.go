...
func (s *Store) migrate() error {
	migrations := []string{
		...
		`CREATE TABLE IF NOT EXISTS alert_channels (
				id INTEGER PRIMARY KEY AUTOINCREMENT,
				name TEXT NOT NULL COLLATE NOCASE UNIQUE,
				channel_type TEXT NOT NULL CHECK (channel_type IN ('webhook', 'telegram', 'email')),
				enabled INTEGER NOT NULL DEFAULT 1,
		...
		`CREATE INDEX IF NOT EXISTS idx_alert_channels_enabled ON alert_channels(enabled, id)`,
		`CREATE TABLE IF NOT EXISTS alert_rules (
				id INTEGER PRIMARY KEY AUTOINCREMENT,
				name TEXT NOT NULL COLLATE NOCASE UNIQUE,
				enabled INTEGER NOT NULL DEFAULT 1,
				event_types_json TEXT NOT NULL DEFAULT '[]' CHECK (json_valid(event_types_json) AND json_type(event_types_json) = 'array'),
		...
		`CREATE INDEX IF NOT EXISTS idx_alert_rule_channels_channel ON alert_rule_channels(channel_id, rule_id)`,
		...
		`CREATE INDEX IF NOT EXISTS idx_alert_deliveries_newest ON alert_deliveries(delivered_at DESC, id DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_alert_deliveries_filter ON alert_deliveries(channel_id, event_type, success, delivered_at DESC)`,
