...
func TestRegisterWebUIServesEmbeddedSPAAndAssets(t *testing.T) {
	gin.SetMode(gin.TestMode)
	router := gin.New()
	...
	router.ServeHTTP(indexRecorder, httptest.NewRequest(http.MethodGet, "/nodes", nil))
	...
	router.ServeHTTP(assetRecorder, httptest.NewRequest(http.MethodGet, "/favicon.png", nil))
