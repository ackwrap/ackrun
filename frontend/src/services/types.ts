export type RuntimeStatus = 'not_installed' | 'no_config' | 'stopped' | 'running' | 'error';
...
export interface GeoLookupResponse {
	...
	dns_server: string;
	resolved_ips: string[];
	geo_assets: Array<{ type: string; name: string; ready: boolean; local_path: string; updated_at: number; error: string }>;
	capabilities: string[];
