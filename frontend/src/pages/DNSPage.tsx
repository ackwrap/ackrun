...
export function DNSPage() {
  ...
  const load = React.useCallback(async () => {
    try {
      setLoading(true);
      const [serversData, rulesData, globalData] = await Promise.all([
        ...
        <>
          ...
          <section className="rounded-[var(--radius-xl)] border border-[var(--border-default)] bg-[linear-gradient(180deg,rgba(20,33,52,0.92),rgba(16,27,43,0.74))] p-5 shadow-[var(--shadow-card)]">
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <label className="block">
                ...
                <input disabled={!globalSettings.enabled} value={globalSettings.final} onChange={e => setGlobalSettings(prev => ({ ...prev, final: e.target.value }))} className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400 disabled:opacity-50" />
              </label>
              <label className="block">
                <span className="text-xs text-[var(--text-tertiary)]">IP 返回策略</span>
                <select disabled={!globalSettings.enabled} value={globalSettings.strategy} onChange={e => setGlobalSettings(prev => ({ ...prev, strategy: e.target.value }))} className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400 disabled:opacity-50">
              ...
              <label className="block">
                ...
                <input disabled={!globalSettings.enabled} value={globalSettings.client_subnet} onChange={e => setGlobalSettings(prev => ({ ...prev, client_subnet: e.target.value }))} placeholder="1.2.3.0/24" className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400 disabled:opacity-50" />
              </label>
          ...
          <section className="rounded-[var(--radius-xl)] border border-[var(--border-default)] bg-[linear-gradient(180deg,rgba(20,33,52,0.92),rgba(16,27,43,0.74))] p-5 shadow-[var(--shadow-card)]">
            ...
            {editingServer !== null && (
              <div className="rounded-xl border border-[var(--border-default)] bg-white/[0.03] p-4">
                <div className="mb-4 flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-white">{editingServer.id ? '编辑 DNS 服务器' : '新增 DNS 服务器'}</h4>
                  <button onClick={resetServerForm} className="rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-1.5 text-xs text-[var(--text-secondary)] hover:text-white">取消</button>
                </div>
                <div className="grid gap-3 md:grid-cols-3">
                  <label className="block">
                    <span className="text-xs text-[var(--text-tertiary)]">Tag（唯一标识）</span>
                    <input value={serverTag} onChange={e => setServerTag(e.target.value)} placeholder="dns_proxy" className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-[var(--text-tertiary)]">类型</span>
                    <select value={serverType} onChange={e => setServerType(e.target.value)} className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-[#152235] px-3 py-2 text-sm text-white outline-none focus:border-blue-400">
                  ...
                  <label className="block">
                    ...
                    <input value={serverAddress} onChange={e => setServerAddress(e.target.value)} placeholder="https://1.1.1.1/dns-query" className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-[var(--text-tertiary)]">Address Resolver</span>
                    <input value={serverAddressResolver} onChange={e => setServerAddressResolver(e.target.value)} placeholder="dns_resolver" className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-[var(--text-tertiary)]">Address Strategy</span>
                    <select value={serverAddressStrategy} onChange={e => setServerAddressStrategy(e.target.value)} className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-[#152235] px-3 py-2 text-sm text-white outline-none focus:border-blue-400">
                  ...
                  <label className="block">
                    ...
                    <select value={serverStrategy} onChange={e => setServerStrategy(e.target.value)} className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-[#152235] px-3 py-2 text-sm text-white outline-none focus:border-blue-400">
                      <option value="">（留空）</option>
                      {strategyOptions.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}
                    </select>
                  </label>
                  <label className="block">
                    ...
                    <input value={serverDetour} onChange={e => setServerDetour(e.target.value)} placeholder="proxy" className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-[var(--text-tertiary)]">Client Subnet</span>
                    <input value={serverClientSubnet} onChange={e => setServerClientSubnet(e.target.value)} placeholder="1.2.3.0/24" className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400" />
                  </label>
                  <div className="flex items-end">
                    <label className="inline-flex items-center gap-2 text-xs text-[var(--text-secondary)]">
          ...
          <section className="rounded-[var(--radius-xl)] border border-[var(--border-default)] bg-[linear-gradient(180deg,rgba(20,33,52,0.92),rgba(16,27,43,0.74))] p-5 shadow-[var(--shadow-card)]">
            ...
            {editingRule !== null && (
              <div className="rounded-xl border border-[var(--border-default)] bg-white/[0.03] p-4">
                <div className="mb-4 flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-white">{editingRule.id ? '编辑 DNS 规则' : '新增 DNS 规则'}</h4>
                  <button onClick={resetRuleForm} className="rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-1.5 text-xs text-[var(--text-secondary)] hover:text-white">取消</button>
                </div>
                <div className="grid gap-3 md:grid-cols-3">
                  <label className="block">
                    <span className="text-xs text-[var(--text-tertiary)]">匹配条件类型</span>
                    <select value={ruleConditionType} onChange={e => setRuleConditionType(e.target.value)} className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-[#152235] px-3 py-2 text-sm text-white outline-none focus:border-blue-400">
                  ...
                  <label className="block md:col-span-2">
                    ...
                    <textarea value={ruleConditionValues} onChange={e => setRuleConditionValues(e.target.value)} rows={3} placeholder={ruleConditionType === 'clash_mode' ? 'rule 或 direct 或 global' : ruleConditionType === 'domain_suffix' ? 'example.com\ngoogle.com' : ruleConditionType === 'geosite' ? 'cn\ngeolocation-!cn' : '每行一个值'} className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 font-mono text-xs text-white outline-none focus:border-blue-400"></textarea>
                  </label>
                  <label className="block">
                    <span className="text-xs text-[var(--text-tertiary)]">DNS Server Tag</span>
                    <input value={ruleServer} onChange={e => setRuleServer(e.target.value)} placeholder="dns_direct" className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-[var(--text-tertiary)]">Rewrite TTL（秒，0 = 不重写）</span>
                    <input type="number" value={ruleRewriteTTL} onChange={e => setRuleRewriteTTL(parseInt(e.target.value) || 0)} className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-[var(--text-tertiary)]">Client Subnet（可选）</span>
                    <input value={ruleClientSubnet} onChange={e => setRuleClientSubnet(e.target.value)} placeholder="1.2.3.0/24" className="mt-1 w-full rounded-md border border-[var(--border-default)] bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-blue-400" />
                  </label>
                </div>
                <div className="mt-3 flex flex-wrap gap-3 text-xs">
                  <label className="inline-flex items-center gap-2 text-[var(--text-secondary)]">
